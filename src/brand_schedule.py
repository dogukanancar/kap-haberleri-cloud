...
def _normalize_time(value: str) -> str:
    ...
    if not match:
        raise ValueError(f"Gecersiz saat formati: {value!r} (ornek: 09:00)")
...
def get_schedule_config() -> BrandScheduleConfig:
    daily_count = max(1, int(repository.get_setting("brand_gunluk_gonderim_sayisi", "1") or "1"))
    times_raw = repository.get_setting("brand_gonderim_saatleri", "").strip()
    if not times_raw:
        times_raw = repository.get_setting("brand_calisma_saati", "09:00")
    send_times = parse_send_times(times_raw)[:daily_count]

    today = _today_istanbul()
    stored_date = repository.get_setting("brand_bugun_gonderim_tarihi", "")
    sent_today: list[str] = []
    if stored_date == today:
        try:
            loaded = json.loads(repository.get_setting("brand_bugun_gonderilen_saatler", "[]") or "[]")
            if isinstance(loaded, list):
                sent_today = [str(item) for item in loaded]
        except json.JSONDecodeError:
            sent_today = []

    return BrandScheduleConfig(
...
def save_schedule_settings(*, send_times: str, daily_count: int) -> BrandScheduleConfig:
    ...
    repository.set_setting("brand_gunluk_gonderim_sayisi", str(daily_count))
    repository.set_setting("brand_gonderim_saatleri", ", ".join(selected_times))
    repository.set_setting("brand_calisma_saati", selected_times[0])
...
def find_due_slot(config: BrandScheduleConfig, now: datetime | None = None) -> str | None:
...
def record_scheduled_send(slot: str | None) -> None:
    ...
    repository.set_setting("brand_bugun_gonderim_tarihi", today)
    repository.set_setting("brand_bugun_gonderilen_saatler", json.dumps(sent_today, ensure_ascii=False))
